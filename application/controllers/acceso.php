...
class Acceso extends CI_Controller {
    ...
    function index()    
    {
        if ($this->session->userdata('aprobado')){redirect('principal','refresh');exit(0);}
        $data=array();        
        $data['titulo']='Acceso a POA Interno';
        $data['contenido']='acceso/ingreso';
        $data['usuario']= array(
                                'name' => 'usuario',
                                'id' => 'usuario',
    ...
    {   
        $this->form_validation->set_rules('usuario', 'Usuario', 'trim|required|strtolower');
	...
	}else
	{
            ...
            $consulta=$this->Usuarios->verificar_acceso($usuario,$clave);
            ...
            { 
              // DEFINIMOS EL RANGO DE FECHAS INICIAL PARA FILTROS DE BÚSQUEDA
               ...
               $tiempo=  getdate(time()-604800); // hace 7 días
              ...
              $sesion = array(
                    ...
                    'cod_estruct'   => ($consulta['codigo']),
                    'nombre_estruct'=> ($consulta['descripcion']),   
                    'fecha_inicial' => $fecha_inicial,
                    'fecha_final'   => $fecha_final,
                    'yearpoa'       => $tiempo['year']+1, // Año próximo
                    'gantt'         => false, // Ver la información en tabla
                    'selector'      => 1, // 1 => Productos, 2 => Proyectos, 3=> Personal, 4=>Insumos
                    'errores'       => ''
                   );
    ...
    {        
        ...
        $data['contenido']='acceso/cambiar_clave';
    ...
    {  
      ...
      {
        die('TRUE'); // SI LA SESION ESTÁ CERRADA DEBE DAR LA ORDEN DE SALIR DEL SISTEMA
      }
      else die('FALSE');                 
    }
}
